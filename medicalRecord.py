...
def get_frame(window):
    frame = Frame(window, name="medicalRecord")
    ...
    n_label = Label(bottom_frame, text="Last Disease")
    ...
    people_choices.heading("name", text="Patient's Name")
    ...
    last_n = Radiobutton(top_frame, text="Last admission", value=0, command=lambda: bottom(1))
...
def display(n=None, name=None, from_date=None, to_date=None):
    medicalRecord_sub_window = Tk()
    medicalRecord_sub_window.title("medicalRecord")

    medicalRecord_table = table.Treeview(medicalRecord_sub_window)
    medicalRecord_table.grid(row=0, column=0, columnspan=6)
    medicalRecord_table["columns"] = ["date", "trans_id", "person_name", "withdrawal", "deposits"]
    medicalRecord_table["show"] = "headings"
    medicalRecord_table.heading("date", text="Date")
    medicalRecord_table.heading("trans_id", text="Transaction Id")
    medicalRecord_table.heading("person_name", text="Patient's Name")
    medicalRecord_table.heading("withdrawal", text="Debit")
    medicalRecord_table.heading("deposits", text="Credit")
    if n != None:
        n = int(n)
        balance = 0
        index = 0
        for trans in transaction.TRANSACTIONS[:n]:
            if trans.type == "Debit":
                ...
                medicalRecord_table.insert("", index,
                ...
                medicalRecord_table.insert("", index,
                                      values=(trans.date, trans.id, trans.person_name, " ", trans.amount))
            index += 1
        medicalRecord_sub_window.mainloop()
            ...
            if from_date1 <= to_date1 and to_date1 <= datetime.now().date():
                ...
                for trans in transaction.TRANSACTIONS:
                    ...
                    if from_date1 <= d <= to_date1:
                        if trans.type == "Debit":
                            medicalRecord_table.insert("", index, values=(
                ...
                medicalRecord_sub_window.destroy()
        else:
